...
select p.nome AS nome_aluno, c.nome AS nome_cursos
from tb_pessoas AS p right join tb_cursos AS c
on p.fk_cursos = c.pk_cursos;
...
select c.nome AS nome_cursos
from tb_cursos join nome_cursos
on c.pk_cursos


-- 3-Exiba todos os cursos e seus respectivos alunos, mesmo que alguns cursos não tenham alunos atribuídos.

select c.nome AS nome_cursos, p.nome AS nome_aluno
from tb_cursos right join nome_cursos, AS tb_pessoas AS nome_pessoas
on p.fk_cursos = c.pk_cursos;

-- 4-Liste todos os cursos, suas descrições e o nome dos alunos, incluindo cursos sem alunos, que ocorreram em 2018.

select c.nome AS nome_cursos, p.nome AS nome_aluno
from tb_cursos left join  nome_cursos AS tb_pessoas  
ON p.fk_cursos = c.pk_cursos;
where c.totaulas > 2018
...
select nome_cursos, nome_nacionalidade,nome_aluno
from tb_cursos AS tb_nacionalidade AS nome_aluno
on p.fk_cursos = c.pk_cursos
...
select nome_cursos AS c.nome AS p.nome, nome_Pessoas 
from tb_cursos AS tb_pessoas AS nome_cursos, nome_pessoas
on p.fk_cursos = c.pk_cursos;



-- 8-Liste todos os cursos com carga horaria acima de 30 e o nome dos alunos, incluindo cursos sem alunos.

select p.nome AS nome_aluno , carga horaria 
from tb_nome left join carga horaria
on p.fk_cursos = c.pk_cursos
where c.totaulas > 30
